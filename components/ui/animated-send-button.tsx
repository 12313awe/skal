...
export function AnimatedSendButton({ 
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`group relative cursor-pointer p-2 w-32 border bg-white rounded-full overflow-hidden text-black text-center font-semibold transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
